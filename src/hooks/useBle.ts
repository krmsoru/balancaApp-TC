...
function useBLE(): BluetoothLowEnergyApi {
  ...
  const connectToDevice = async (device: Device) => {
    try {
      ...
      startStreamingData(deviceConnection).then((e) => console.log(e));
    } catch (e) {
  ...
  const disconnectFromDevice = () => {
    try {
      if (connectedDevice) {
        bleManager.cancelDeviceConnection(connectedDevice.id).then((e) => {
  ...
  const onUpdate = (
    ...
    try {
      ...
      if (!isNaN(weightValue)) {
        ...
        console.error("VALOR DE PESO INVALIDO", weightData);
      }
    } catch (error) {
      console.log(`ERRO -> ERRO ${error}`);
    }
  ...
  const postRequest = (device: Device) => {
    try {
      ...
      console.log("ERRO -> ERRO AO ENVIAR RESPOSTA");
    }
  };
