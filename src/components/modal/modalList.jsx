...
import { Text, View, TouchableOpacity, SafeAreaView, FlatList, TextInput, Pressable } from 'react-native';
...
export default function ModalFoods({ handClose, handleSelectedFood }) {
    ...
    return (
        ...
        >
            ...
            >
                <TextInput
                    style={[Styles.searchInput, { fontSize: 20, borderWidth: 2, borderColor: '#e6e6e6', padding: 10 }]}
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={[Styles.radioButtonContainer, { borderColor: selectedFood && selectedFood.id === item.id ? '#007AFF' : '#e6e6e6' }]} onPress={() => selectFood(item)}>
                            <RadioButton
                                value={item.nome}
                                status={selectedFood === item ? 'checked' : 'unchecked'}
                                onPress={() => selectFood(item)}
                            />
                            <TouchableOpacity style={{ marginLeft: 10 }} onPress={() => selectFood(item)}>
                                ...
                                {selectedFood && selectedFood.id === item.id && (
                                    <View style={{ paddingVertical: 5, }}>
                                        ...
                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            ...
                                            <Text style={Styles.nutrientText}>{item.gordura_total_g} g</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Gordura Saturada: </Text>
                                            <Text style={Styles.nutrientText}>{item.gordura_saturada_g} g</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Proteina: </Text>
                                            <Text style={Styles.nutrientText}>{item.proteina_g} g</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Sodio: </Text>
                                            <Text style={Styles.nutrientText}> {item.sodio_mg} mg</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Potassio: </Text>
                                            <Text style={Styles.nutrientText}> {item.potassio_mg} mg</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Colesterol: </Text>
                                            <Text style={Styles.nutrientText}> {item.colesterol_mg} mg</Text>
                                        </View>

                                        <View style={{ flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={Styles.nutrientText}>Carboidratos: </Text>
                                            <Text style={Styles.nutrientText}> {item.carboidratos_total_g} g</Text>
                                        </View>
                                        ...
                                        <View style={{ flex: 1, flexDirection: 'row', marginHorizontal: 10, justifyContent: 'space-between' }}>
                                            <Text style={[Styles.nutrientText, { justifyContent: 'flex-start' }]}>Açucar: </Text>
                                            <Text style={[Styles.nutrientText, { justifyContent: 'flex-end' }]}> {item.acucar_g} g</Text>
                                        </View>
